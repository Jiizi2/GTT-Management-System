      ...
      try {
        await this.prisma.itineraryItem.create({
          data: {
            ...
            transportMode: payload.transportMode?.trim() || null,
            ...
            busCount: Math.max(0, Math.floor(payload.busCount ?? (payload.requiresBus ? 1 : 0))),
            requiresBus: Math.max(0, Math.floor(payload.busCount ?? (payload.requiresBus ? 1 : 0))) > 0,
