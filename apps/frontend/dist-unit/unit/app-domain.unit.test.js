...
import { buildChecklistActivityLabel, buildChecklistItemsFromGroups, buildItineraryItemFromEditForm, buildTransferTrainSummary, buildVisaTrackingRowsFromGroups, createDummyOverviewGroups, createEditScheduleForm, createEmptyChecklistDraft, createEmptyChecklistDriverProfile, createInitialInputItineraryForm, createInitialNoteForm, createInitialScheduleForm, createNewGroupAgreementForm, createNewGroupRaudhahForm, createNoteItems, createScheduleMeta, detectCityFromText, escapeHtml, expandInputTransferTrainItems, expandTransferTrainItineraryItems, formatAprilDisplayDate, formatAprilIsoDate, formatChecklistCopyDate, formatRouteSummary, formatScheduleDate, formatScheduleTime, getChecklistRangeDates, getChecklistDayLabel, getLocalIsoDateWithOffset, getMinimumBusCountForPax, getRouteFieldConfigByCategory, getScheduleTypeOption, getStatusByTone, getTransferTrainSegmentCategory, getItineraryIsoDate, hasIncompleteTransferTrainFields, includesKnownKeyword, inferCategoryKey, inferCityTourCity, isCityTourActivityType, isDepartureActivityType, isFlightActivityType, isFridayDate, isTransferActivityType, normalizeGroupStatus, normalizeSaudiCityValue, overviewDummySeeds, parseDisplayDateToIso, parseTimeForInput, resolveGroupToneByItinerary, resolveTotalBusCount, resolveValidRaudhahAppointments, scrollToTop, shouldShowFridayCityTourWarning, sortInputItineraryItems, } from "../shared/app-domain.js";
function createBaseGroup(overrides = {}) {
...
function createBaseItineraryItem(overrides = {}) {
    return {
        date: "10 Apr",
        year: "2026",
        category: "City Tour",
...
function testResolveValidRaudhahAppointmentsNormalization() {
...
function testRouteHelpersForCategorySpecificBehavior() {
...
function testTransferTrainExpansionCreatesTwoChecklistSegments() {
    const items = [
        {
            id: "unit-transfer",
            date: "2099-01-03",
...
function testBuildVisaTrackingRowsUsesItineraryBoundariesAndStatuses() {
    const group = createBaseGroup({
        code: "UNIT-VISA",
        itinerary: [
            {
...
function testBuildChecklistItemsFiltersDateWindowAndUsesDeparturePickupTime() {
    ...
    assert.equal(transferItems.some((entry) => entry.activity === "Transfer - Train Departure" && entry.scheduledTime === "09:00"), true);
    assert.equal(transferItems.some((entry) => entry.activity === "Transfer - Arrival Station Pickup" && entry.scheduledTime === "10:20"), true);
}
function testOverviewAndStatusNormalizationHelpers() {
    ...
    assert.equal(resolveGroupToneByItinerary([
    ...
    const normalized = normalizeGroupStatus(createBaseGroup({
        code: "UNIT-NORMALIZE",
        status: "Unknown",
        tone: "inactive",
        pax: 120,
...
function testFormFactoryAndCategoryHelpers() {
    ...
    assert.equal(hasIncompleteTransferTrainFields({
        category: "arrival",
        transferByTrain: true,
        trainDepartureTime: "",
        destinationPickupTime: "",
    ...
    assert.equal(buildTransferTrainSummary({
        category: "transfer",
        transferByTrain: true,
...
function testScheduleEditingAndCityInferenceHelpers() {
    ...
    assert.equal(createScheduleMeta({
        time: " ",
        from: "",
        to: "",
    ...
    assert.equal(inferCityTourCity(createBaseItineraryItem({
        cityTourCity: "",
        from: "",
        to: "",
...
function testDisplayChecklistAndScrollHelpers() {
    ...
    assert.equal(getItineraryIsoDate(createBaseItineraryItem({ isoDate: undefined, date: "2 Apr", year: "2026" })), "2026-04-02");
    assert.equal(buildChecklistActivityLabel(createBaseItineraryItem({ category: "Departure" }), "departure"), "Departure");
    assert.equal(buildChecklistActivityLabel(createBaseItineraryItem({
        category: "City Tour",
        cityTourCity: "",
    }), "city-tour"), "City Tour");
    assert.equal(buildChecklistActivityLabel(createBaseItineraryItem({
