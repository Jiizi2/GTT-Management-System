...
const MATERIAL_SYMBOLS_FONT_SRC_PATTERN =
  /url\((["'])\.\/fonts\/material-symbols-outlined\.woff2(?:\?v=(?<version>[a-f0-9]+))?\1\)\s*format\(["']woff2["']\)/u;
...
function buildMaterialSymbolsFontSource(fontVersion) {
  return `url("./fonts/material-symbols-outlined.woff2?v=${fontVersion}") format("woff2")`;
}
...
async function verifyFontsCssFontVersion() {
  ...
  if (fontSourceMatch.groups?.version !== fontVersion) {
    throw new Error(
      "Material Symbols font cache-busting URL is out of date. Run `npm run assets:icons --workspace frontend`.",
    );
  }
}
