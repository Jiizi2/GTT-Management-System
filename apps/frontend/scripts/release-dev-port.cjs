...
function resolvePort() {
  ...
  if (!Number.isInteger(parsedPort) || parsedPort < 1 || parsedPort > 65_535) {
    console.warn(`[frontend-port-guard] Ignoring invalid PORT='${process.env.PORT}', falling back to ${DEFAULT_PORT}.`);
    return DEFAULT_PORT;
  }
...
function readListeningPids(port) {
  try {
    return process.platform === "win32" ? readListeningPidsWindows(port) : readListeningPidsPosix(port);
  } catch {
    return [];
  }
}
...
function main() {
  ...
  console.log(`[frontend-port-guard] Port ${port} is busy (PID: ${pids.join(", ")}). Attempting to free it...`);
  ...
  for (const pid of pids) {
    ...
    if (!isNodeProcessName(processName)) {
      failedPids.push({
        pid,
        error: new Error(`Refusing to terminate non-node process '${processName || "unknown"}' using port ${port}.`),
      });
      continue;
    }
  ...
  console.log(`[frontend-port-guard] Port ${port} is now free.`);
}
...
try {
  ...
  console.error(`[frontend-port-guard] ${message}`);
  process.exit(1);
}
